...
// ── 5. Connect using native sqlsrv if PDO not available ──────────────────────
try {
    if (DB_TYPE === 'sqlsrv' && function_exists('sqlsrv_connect')) {
        // Native SQL Server connection (no PDO required)
        $connectionInfo = [
            "Database" => DB_NAME,
            "UID" => DB_USER,
            "PWD" => DB_PASS,
        ...
        // Create PDO-like wrapper for compatibility
        $pdo = new class($conn) {
            private $conn;
            ...
            public function query($sql) {
                ...
                if ($stmt === false) {
                    throw new PDOException("Query failed: " . print_r(sqlsrv_errors(), true));
                }
                return new class($stmt) {
                    private $stmt;
                    public function __construct($stmt) { $this->stmt = $stmt; }
                    public function fetch($mode = null) { return sqlsrv_fetch_array($this->stmt, SQLSRV_FETCH_ASSOC); }
            ...
            public function prepare($sql) {
                return new class($this->conn, $sql) {
                    ...
                    public function execute($params = []) {
                        ...
                        if ($stmt === false) {
                            throw new PDOException("Execute failed: " . print_r(sqlsrv_errors(), true));
                        }
                        return new class($stmt) {
                            private $stmt;
                            public function __construct($stmt) { $this->stmt = $stmt; }
    ...
    } elseif (DB_TYPE === 'sqlsrv' && class_exists('PDO')) {
        // PDO SQL Server connection
        $pdo = new PDO(
